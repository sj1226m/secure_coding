        ...
        if st.session_state.user["role"] == 'admin':
            ...
            if choice == 'Home':
                ...
                with st.form(key='add_product_form'):
                    ...
                    if submit_button:
                        ...
                        # 응답 코드 확인 로직 추가
                        if add_product_response.json()["status_code"] == 200:
                            st.success(add_product_response.json()["message"])
                        elif add_product_response.json()["status_code"] == 400:
                            st.error(add_product_response.json()["message"])
                        else:
            ...
            # 상품 삭제 페이지 추가
                ...
                with st.form(key='delete_product_form'):
                    ...
                    if submit_button:
                        delete_product_response = requests.delete(f'http://localhost:8000/delete_product/{name}')
            ...
            menu = ['Home', 'Buy Products', 'My Page', 'Purchase History']
            choice = st.sidebar.selectbox('Menu', menu)
            ...
            if choice == 'Home':
                ...
                if st.button('Buy'):
                    ...
                    purchase_response = requests.post(f'http://localhost:8000/purchase/{selected_product}', params={"user_name": user_name})
        
                    if purchase_response.status_code == 200:
                        st.success(f'You have successfully purchased {selected_product}! Address: {user_address}, Payment Info: {user_payment_info}')
                    else:
                        st.error("Failed to purchase the product.")

            elif choice == 'Purchase History':
                    st.subheader('Purchase History')
                    user_name = st.session_state.user["username"]
                    response = requests.get(f'http://localhost:8000/purchase_history/{user_name}')
                    history = response.json()
                    if response.status_code == 200:
                        for purchase in history:
                            st.write(f"Name: {purchase['product_name']}, Time: {purchase['purchase_time']}")
                    else:
